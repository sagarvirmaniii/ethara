...
const STATS = (tasks) => [
  { label: 'Total',       value: tasks.length,                                           accent: 'border-l-indigo-500',  val: 'text-indigo-600'  },
  { label: 'Completed',   value: tasks.filter((t) => t.status === 'Completed').length,   accent: 'border-l-emerald-500', val: 'text-emerald-600' },
  { label: 'In Progress', value: tasks.filter((t) => t.status === 'In Progress').length, accent: 'border-l-blue-500',    val: 'text-blue-600'    },
  { label: 'Todo',        value: tasks.filter((t) => t.status === 'Todo').length,         accent: 'border-l-gray-300',    val: 'text-gray-600'    },
];
  ...
  return (
    <Layout>
      ...
      <div className="mb-8">
        <Link
          to="/projects"
          className="btn-press inline-flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-indigo-600 mb-3"
        >
        ...
        {project.description && (
          <p className="text-sm text-gray-500 mt-1 max-w-2xl leading-relaxed">{project.description}</p>
        )}
      </div>
      ...
      <div className="stagger grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {STATS(tasks).map((s) => (
          <div key={s.label} className={`card-hover bg-white rounded-xl border border-gray-100 border-l-4 ${s.accent} shadow-sm px-5 py-4`}>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{s.label}</p>
            <p className={`text-2xl font-bold mt-1.5 tabular-nums ${s.val}`}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link
              to="/tasks"
              className="btn-press inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-700"
            >
              Manage Tasks
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <dl className="space-y-3 text-sm">
              <div>
                <dt className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Created by</dt>
                <dd className="text-gray-700 font-medium">{project.createdBy?.name}</dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Created on</dt>
                <dd className="text-gray-700">{new Date(project.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-0.5">Members</dt>
